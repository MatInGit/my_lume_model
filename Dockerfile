...
COPY environment.yml /my_lume_model/environment.yml

RUN apt-get update && \
    apt-get install -y gcc git build-essential

RUN mamba install -c conda-forge conda-pack && \
  mamba env create -f /my_lume_model/environment.yml

# Use conda-pack to create a  enviornment in /venv:
RUN conda-pack -n my_lume_model -o /tmp/env.tar && \
...
RUN source /venv/bin/activate && \
  python -m pip install /my_lume_model
